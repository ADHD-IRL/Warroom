        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2.5 border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 rounded-lg font-display uppercase text-sm transition-all"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isPending}
              className="flex-1 px-4 py-2.5 bg-primary text-primary-foreground font-display uppercase text-sm font-bold rounded-lg hover:bg-primary/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {domains?.map((domain) => (
            <div
              key={domain.id}
              className="bg-card rounded-xl p-6 border shadow-lg relative overflow-hidden group"
            ...
            >
              ...
              <button
                ...
                className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-red-400 p-1.5 rounded hover:bg-red-400/10"
